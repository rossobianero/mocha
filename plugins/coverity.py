# Coverity plugin (SAST) using local analysis tools (placeholder logic)
...
class CoverityPlugin(ScannerPlugin):
    ...
    def scan(self, repo_dir, c):
        # In a real integration: cov-build + cov-analyze + cov-format-errors
        # Stub a simple finding for initial wiring
        fake = Finding(
            id="RESOURCE_LEAK",
            tool="coverity", kind="SAST", severity="medium",
            cwe=["CWE-772"],
            file="src/example.c", start_line=42,
